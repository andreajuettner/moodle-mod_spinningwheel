...
/**
 * Renderable for the Spinning Wheel view page.
 */
class view_page implements renderable, templatable {
    /** @var stdClass The spinningwheel instance record. */
    private stdClass $spinningwheel;
    /** @var array The active wheel entries. */
    private array $entries;
    /** @var bool Whether the current user can spin. */
    private bool $canspin;
    /** @var bool Whether the current user can view the history. */
    private bool $canviewhistory;
    /** @var int The course module ID. */
    private int $cmid;
    /** @var int The current group ID. */
    private int $groupid;
    /** @var bool Whether this is rendered in the embed view. */
    ...
    public function export_for_template(renderer_base $output): stdClass {
        ...
        if ($this->spinningwheel->entrysource == SPINNINGWHEEL_SOURCE_ACTIVITIES && !$pendingactivity) {
            ...
            } else if ($spinnablecount === 0) {
                $data->canspin = false;
                $data->allcompleted = true;
            }
        }
        $data->canviewhistory = $this->canviewhistory;
        $data->canclearhistory = has_capability(
            'mod/spinningwheel:clearhistory',
